...
void TDBProcess::OnExecSQL(const ServiceIdentifier&sender,UserTransferMessage& utm)
{
	...
	utm.setReq(m_owner->myIdentifier());
...
#	else
	...
	utm.setReturn( 0 );
#	endif
	PrintLog("SEND ResExecSQL  fieldcount =%d, rowcount=%d,appport=%d\n",fieldcount,rowcount,utm.getRes().m_appport);
	if (AppPort::_apHttpServer == utm.getRes().m_appport)
	{
		m_owner->postSelfMessage(utm);
	}
	else if (AppPort::_apSlee== utm.getRes().m_appport)
	{
		m_owner->postMessage(sender, utm);
	}
