...
struct CDBProcInfo
{
	u32	m_currIndex;
	bool m_connected;
	int m_countMsg;  //计数器,如果大于最大次数异常后才开始重连
...
private:
	...
	void	HandleRespExecSQL(const ServiceIdentifier& sender, UserTransferMessage& utm);
	void	HandleRespRegister(const ServiceIdentifier& sender, UserTransferMessage& utm);
	void	HandleReqRegister(const ServiceIdentifier& sender, UserTransferMessage& utm);
	void    HandleStartFlowEx(const ServiceIdentifier& sender, UserTransferMessage& utm);
public:
