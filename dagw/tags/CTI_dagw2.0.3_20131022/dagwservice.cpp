...
void TDBGWService::HandleExecSQL(const ServiceIdentifier& sender, UserTransferMessage& utm)
{
	...
	{
		g_dagwlog.print("this dbconnection is not conect,must reconnect!\n");
...
void TDBGWService::reconnect(string sourceName)
{
	...
	m_countMsg++; 
	g_dagwlog.print("m_countMsg=%d\n", m_countMsg); 

	if (1!=m_countMsg%DAGW_MAX_COUNT_MSG)
	{
		g_dagwlog.print("[%d] Reconnect(%s) return\n", m_countMsg,sourceName.c_str()); 
		return;
	}
	try
	{ 
		...
		{	
			...
			if (it!=this->m_mapProcess.end())
			{
				...
				//vector[TDBprocess]遍历出来，释放query
				if (pDBProcInfo->m_connected)
				{
				...
				if(sourceName==g_cfgData.m_vctConnection[i].m_DB_Server)
				{
